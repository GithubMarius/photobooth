...
class ExtendedScreen:
    ...
    def dispMode(self,mode = 1, noup = 1):
        
        self.fillbg()
        
        if mode == 1:
        
            dispButton(self.space,self,1,mode = 1)
            dispButton(self.left,self,3,mode = 1)
            dispButton(self.right,self,4,mode = 1)
...
def dispObj(obj,ExtScreen,color = white,objtype = 1, RectIn = None): #displays an object depending on the wanted position
...
def dispPreview(obj,ExtScreen,pos = 1):
...
def dispButton(obj,ExtScreen,pos = 1,mode = 1):
...
def dispImg(ExtScreen,img_num):
    ...
    ExtScreen.dispMode(1,1)
    dispObj(img,ExtScreen,white,0)
