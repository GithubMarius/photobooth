...
# Import of Modules
    ...
    from photobooth_settings            import black, white, red, num_keys, framerate, COM, lbl_choose_keys, up, down, right, left, space, t_disp, t_swipe, t_rand, t_move, t_hold, myfontsmall, yellow, externalFlash
    from photobooth_imgproc             import countdown_img, img_combine
    from photobooth_camcon              import post_method, start_liveview, take_picture, readImgBytes
    ...
    #Get Standard Iso            
    resp = post_method('getIsoSpeedRate')   
    respJs = resp.json()
    takeIso = str(respJs.get('result')[0])
    
    #Get Standard F            
    resp = post_method('getFNumber')   
    respJs = resp.json()
    takeF = str(respJs.get('result')[0])
    
    #Get Shutter Speed            
    resp = post_method('getShutterSpeed')   
    respJs = resp.json()
    ...
    while True:
        ...
        if i == 0:
            ...
            img_1 = countdown_img(ExtScreen,Ser,link,Clock,1,takeIso,takeF,takeS)
            ...
            img_2 = countdown_img(ExtScreen,Ser,link,Clock,2,takeIso,takeF,takeS)
            #------------------------------------ img_2 = Image.open('img/test.jpg')
            ExtScreen.fillbg()
            pygame.display.flip()
             
            img_3 = countdown_img(ExtScreen,Ser,link,Clock,3,takeIso,takeF,takeS)
            #------------------------------------ img_3 = Image.open('img/test.jpg')
            ExtScreen.fillbg()
            pygame.display.flip()
             
            img_4 = countdown_img(ExtScreen,Ser,link,Clock,4,takeIso,takeF,takeS)
